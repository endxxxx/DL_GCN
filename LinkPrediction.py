...
from Dataset import *
from GCN_Network import GCNForLinkPrediction
from utils import set_random_seed, plot_loss_with_acc, set_pic_save_path
from sklearn.metrics import roc_auc_score


def generate_tensor_adjacency_for_link(edge_index, drop_edge=0.05, self_loop=True):
    if drop_edge >= 1.0:
...
def train():
        ...
        for epoch in range(epoch_num):
            ...
            train_acc, _ = test(train_pos_edge_index, train_neg_edge_index)  # 计算当前模型训练集上的准确率  调用test函数
            val_acc, val_loss = test(validate_pos_edge_index, validate_neg_edge_index)  # 计算当前模型在验证集上的准确率
...
def test(pos_edge_index, neg_edge_index):
        ...
        with torch.no_grad():  # 测试过程不计算梯度
            tensor_adjacency = generate_tensor_adjacency_for_link(pos_edge_index).to(device)
            ...
            loss = F.binary_cross_entropy_with_logits(logits, labels)
            accuracy = roc_auc_score(labels.cpu(), logits.cpu())
...
if __name__ == "__main__":
    ...
    model = GCNForLinkPrediction(hidden_layer_dim=hidden_layer_dim, num_of_hidden_layer=layer_num,
    ...
    test_acc, _ = test(test_pos_edge_index, test_neg_edge_index)
    ...
    plot_loss_with_acc(loss_history=loss, val_loss_history=val_loss, val_acc_history=val_acc, plot_path=plot_path)
